...
def conv3x3(conv, in_planes, out_planes, stride=1, groups=1, dilation=1):
...
def conv1x1(conv, in_planes, out_planes, stride=1):
    """1x1 convolution"""
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, conv, conv_1x1, stride=1, downsample=None, groups=1,
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, conv, conv_1x1, stride=1, downsample=None, groups=1,
        ...
        self.conv1 = conv1x1(conv_1x1, inplanes, width)
        self.bn1 = norm_layer(width)
        self.conv2 = conv3x3(conv, width, width, stride, groups, dilation)
        self.bn2 = norm_layer(width)
        self.conv3 = conv1x1(conv_1x1, width, planes * self.expansion)
...
from .dense_conv import DenseConv2D
class ResNet(nn.Module):

    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
                groups=1, width_per_group=64, replace_stride_with_dilation=None,
                norm_layer=None, imagenet=False, use_sparse_conv=False):
        super(ResNet, self).__init__()
        if not use_sparse_conv:
            conv = nn.Conv2d
            conv_1x1 = nn.Conv2d
        ...
        self.layer1 = self._make_layer(block, 64, layers[0], conv, conv_1x1)
        self.layer2 = self._make_layer(block, 128, layers[1], conv, conv_1x1, stride=2,
                                    dilate=replace_stride_with_dilation[0])
        self.layer3 = self._make_layer(block, 256, layers[2], conv, conv_1x1, stride=2,
                                    dilate=replace_stride_with_dilation[1])
        self.layer4 = self._make_layer(block, 512, layers[3], conv, conv_1x1, stride=2,
    ...
    def _make_layer(self, block, planes, blocks, conv, conv_1x1, stride=1, dilate=False):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                conv1x1(self.conv_1x1, self.inplanes, planes * block.expansion, stride),
                norm_layer(planes * block.expansion),
            )

        layers = []
        layers.append(block(self.inplanes, planes, conv, conv_1x1, stride, downsample, self.groups,
                            self.base_width, previous_dilation, norm_layer))
        self.inplanes = planes * block.expansion
        for _ in range(1, blocks):
            layers.append(block(self.inplanes, planes, conv, conv_1x1, groups=self.groups,
